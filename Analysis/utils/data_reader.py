...
class Data:
	"""
	one piece of data, corresponding to one .txt file
	"""
	...
	def read(self, file_path, show_msg=True):
		...
		:param show_msg: bool, whether to log in console
		:return:
		"""
		...
		self.description = lines[0]  # the first line as description words of each experiment
		# get three kinds of timestamps as follows and catch possible error
		try:
			...
			exit(1)
		# frame data starting from line 5
		for line in lines[4:]:
	...
	def get_list(self, type_name) -> FrameList:
		"""
		get sensor data list by sensor name

		:param type_name: name string of a sensor type
		:return: sensor's data in frame list form
		"""
