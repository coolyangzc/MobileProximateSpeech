...
def reduce_noise_mfcc_up(y, sr):
	...
	speech_booster = AudioEffectsChain(). \
		lowshelf(frequency=min_hz * (-1), gain=12.0,
...
def enhance(y):
	apply_audio_effects = AudioEffectsChain().lowshelf(gain=10.0, frequency=260, slope=0.1). \
		reverb(reverberance=25, hf_damping=5, room_scale=5, stereo_depth=50, pre_delay=20, wet_gain=0,
...
def output_file(destination, filename, y, sr, ext=""):
	destination = os.path.join(destination, filename.split('.')[0] + ext + '.wav')
...
if __name__ == '__main__':
	...
	# reading a file
	filename = 'jwy.wav'
	y0, sr0 = librosa.load(filename, sr=16000)
	...
	y_reduced_power_and_centroid_mb, time_trimmed = trim_silence(y_reduced_power_and_centroid_mb)
	print(time_trimmed)

	# generating output file [1]
	output_file('out/', noisedname, y_reduced_power, sr, '_pwr')
	output_file('out/', noisedname, y_reduced_centroid_s, sr, '_ctr_s')
	output_file('out/', noisedname, y_reduced_centroid_mb, sr, '_ctr_mb')
	output_file('out/', noisedname, y_reduced_mfcc_up, sr, '_mfcc_up')
	output_file('out/', noisedname, y_reduced_mfcc_down, sr, '_mfcc_down')
	output_file('out/', noisedname, y_reduced_median, sr, '_median')
	output_file('out/', noisedname, y_reduced_power_and_centroid_mb, sr, '_power+ctr_mb')
	output_file('out/', noisedname, y, sr, '_org')
	output_file('out/', filename, y0, sr0, '_org')
