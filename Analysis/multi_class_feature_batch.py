...
from multi_class_feature import find_suitable_end, get_vad_chunks
...
def extract_motion_feature(start_time, end_time, data, time_delta):
	start_time *= 1000
	end_time *= 1000
...
def extract_capa_feature(start_time, end_time, data, time_delta):
	start_time *= 1000
	end_time *= 1000
	s, e = start_time, end_time + time_delta
	m = (s + e) / 2
...
def calc_motion_capa_data(file_name, file_dir):
	...
	if calc_motion:
		for t in time_point:
			extract_motion_feature(0.05, end, d, t)

	if calc_capa:
		for t in time_point:
			extract_capa_feature(0.05, end, d, t)
...
def calc_voice_data(file_name, file_dir):
	...
	try:
		...
		return
	# nchannels, sample_width,  = wavefile.getnchannels(), wavefile.getsampwidth()
