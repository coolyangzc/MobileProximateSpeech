...
public class VIPDemoActivity extends Activity implements SensorEventListener {
    ...
    public void onSensorChanged(SensorEvent sensorEvent) {
        ...
        switch (type) {
            case Sensor.TYPE_LINEAR_ACCELERATION:
                ...
                if (!orientationOK || sensorEvent.timestamp - triggerTimestamp <= 1000 * 1000000L)
                    return;
    ...
    private boolean check_LINEAR_ACCELERATION() {
        ...
        float[] maxDistance = new float[3];
        float[] maxLinearAcc = new float[3];
        for (Frame f: f_list) {
            if (t - f.timestamp > 600 * 1000000L)
                continue;
            float[] v = f.values.clone();
            for (int j = 0; j < dataOffset[id].values.length; ++j)
                v[j] -= dataOffset[id].values[j];
            for (int i = 0; i < v.length; ++i) {
                maxLinearAcc[i] = Math.max(maxLinearAcc[i], Math.abs(v[i]));
                sum[i] += v[i];
                distance[i] += sum[i];
                maxDistance[i] = Math.max(maxDistance[i], Math.abs(distance[i]));
            }
        }
        boolean rapid = false;
        /*
         ...
         */
        for (int i = 0; i < 3; ++i)
            if (maxDistance[i] > 1000 || maxDistance[i] > 600 && maxLinearAcc[i] > 8) {
                rapid = true;
                break;
            }
        ...
        sb.append("MaxLinearAcc: " + String.format(Locale.US, "%.1f %.1f %.1f\n",
                maxLinearAcc[0], maxLinearAcc[1], maxLinearAcc[2]));
